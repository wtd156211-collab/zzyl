...
public class HealthAssessmentServiceImpl extends ServiceImpl<HealthAssessmentMapper, HealthAssessment> implements IHealthAssessmentService
{
    ...
    private Long saveHealthAssessment(HealthReportVo healthReportVo, HealthAssessment healthAssessment) {
        ...
        double score = healthReportVo.getHealthAssessment().getHealthIndex(); // 获取健康指数
        healthAssessment.setHealthScore(String.valueOf(score));

        String riskLevel = healthReportVo.getHealthAssessment().getRiskLevel();
        ...
        //设置护理等级
        String nursingLevel = getNursingLevel(score);
        healthAssessment.setNursingLevelName(nursingLevel);

        //设置入住状态
        healthAssessment.setAdmissionStatus(1);

        //设置总检日期
        String totalCheckDate = healthReportVo.getTotalCheckDate();
        healthAssessment.setTotalCheckDate(totalCheckDate);
        ...
        return save(healthAssessment) ? healthAssessment.getId() : null;
    }
     ...
     * @return
     */
    private String getNursingLevel(double score) {
        if(score >= 90){
            return "四级护理等级";
        }
        if(score >= 80){
    ...
    private String getPrompt(String idCard) {
        ...
        String content = (String) redisTemplate.opsForHash().get(CacheConstants.HEALTH_REPORT_KEY, idCard);
        //判断数据是否为空
        if(StringUtils.isEmpty(content)){
            throw new BaseException("体检报告解析失败,请重新上传体检报告！");
        }
        //从Redis中删除数据
        redisTemplate.opsForHash().delete(CacheConstants.HEALTH_REPORT_KEY, idCard);
