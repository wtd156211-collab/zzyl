...
public class NursingLevelController extends BaseController
{
    ...
    @GetMapping("/list")
    public TableDataInfo<List<NursingLevelVo>> list(@ApiParam(value = "护理等级查询条件") NursingLevelVo vo)
    {
        startPage();
        List<NursingLevelVo> list = nursingLevelService.selectNursingLevelVoList(vo);
    ...
    @PostMapping("/export")
    public void export(@ApiParam("护理等级导出的查询条件对象") HttpServletResponse response, NursingLevel nursingLevel)
    {
	...
	@DeleteMapping("/{ids}")
    public AjaxResult remove(@PathVariable @ApiParam("删除的护理等级ID") Long[] ids)
    {
